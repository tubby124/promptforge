...
const CATEGORY_GUIDANCE = {
  ...
  'image-edit':
    `OUTPUT FORMAT: a precise image-editing prompt for a vision/image model using the attached reference image(s). Include the requested change, what to preserve, what to remove or avoid, output size/aspect ratio if known, print/export constraints if relevant, and a short proof-check instruction. Do not ask the model to redraw unchanged areas unless necessary. Do not invent text, logos, prices, or contact details that are not supplied.`,
  code:
...
// Resolve the effective mode. Image-gen/image-edit always force deep.
export function resolveMode({ mode, category }) {
  if (category === 'image-gen' || category === 'image-edit') return 'deep';
...
function renderPackHints(pack, category) {
  ...
  if ((category === 'image-gen' || category === 'image-edit') && pack.image_styles) {
    ...
    out.push('Always layer: subject + composition + camera/lens + lighting + color/mood + style anchor + quality boosters. For image-edit, also say what must remain unchanged from the reference image.');
  }
